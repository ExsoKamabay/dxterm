package com.dracxterm.guest

import android.content.Context
import android.util.Log
import com.dracxterm.Bootstrap
import com.dracxterm.rootfs.DpkgRecoveryEngine
...
object GuestBackend {
    ...
    private fun selfTest(ctx: Context, rootfs: File): Decision {
        ...
        reader.start()
        if (!awaitExit(proc, SELFTEST_TIMEOUT_MS)) {
            killNow(proc)
    ...
    /**
     * True when [proc] has exited within [millis].
     *
     * Process.waitFor(timeout) arrived in API 26 and minSdk here is 24, so on Android 7 the call
     * would be a NoSuchMethodError inside the backend self-test -- on the one path whose whole
     * purpose is to decide, safely, whether this device can run VHDP. Below API 26 the exit is
     * polled instead: exitValue() throws IllegalThreadStateException while the process is alive.
     */
    private fun awaitExit(proc: Process, millis: Long): Boolean {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            return proc.waitFor(millis, java.util.concurrent.TimeUnit.MILLISECONDS)
        }
        val deadline = android.os.SystemClock.uptimeMillis() + millis
        while (true) {
            if (runCatching { proc.exitValue() }.isSuccess) return true
            if (android.os.SystemClock.uptimeMillis() >= deadline) return false
            Thread.sleep(50)
        }
    }

    /** destroyForcibly() is API 26; Android's destroy() already sends SIGKILL, so it is the same
     *  thing on the older releases. */
    private fun killNow(proc: Process) {
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
