...
object Bootstrap {
    ...
    fun prepare(ctx: Context): Spec {
        ...
        val env = when {
            ...
            backend == com.dracxterm.guest.GuestBackend.Backend.VHDP -> arrayOf(
                "HOME=$home",
                "PWD=$home",
                "TMPDIR=$tmp",
                "TERM=xterm-256color",
        ...
        val argv = if (hasGuest) {
            ...
            com.dracxterm.guest.GuestBackend.launchArgv(ctx, rootfs, guestEnv)
        } else {
     ...
     */
    fun fakerootShell(ctx: Context, rootfs: String, body: String): Pair<Array<String>, Array<String>> {
        // The login umask, as the terminal session has it (Pty.cpp): package scripts run here too.
        val script = "umask 022\n$body"
        val nativeLib = ctx.applicationInfo.nativeLibraryDir
        val files = ctx.filesDir.absolutePath
        ...
        if (com.dracxterm.guest.GuestBackend.chosenBackend(ctx, File(rootfs)) == com.dracxterm.guest.GuestBackend.Backend.VHDP) {
     ...
     * [com.dracxterm.guest.GuestBackend.selectBackend]).
     */
    fun prootArgv(ctx: Context, rootfs: String): Array<String> {
        ...
        for ((h, g) in com.dracxterm.guest.GuestBackend.systemBinds()) bind(h, g)
    ...
    fun vhdpArgv(ctx: Context, rootfs: String, guestEnv: Array<String>): Array<String> {
        ...
        for ((h, g) in com.dracxterm.guest.GuestBackend.systemBinds()) {
