...
struct FeatureEntry {
    ...
    SupportStatus terminal_unprivileged; // Android Termux/adb shell, same-arch
...
// First-boot configuration of an already-extracted rootfs: proot bind mountpoints, DNS/hosts,
...
// decision and the recovery script; the caller runs `script` through the proot execution backend
// (running dpkg needs to exec guest binaries, which VHDP cannot do in an app process).
...
// the device, plus which host trees (/dev, /proc, /sys) to project into the guest. The execution
// backend (proot) applies the plan's binds. Returns JSON {host, system_binds[], status}.
