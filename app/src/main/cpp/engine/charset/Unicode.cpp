...
// Binary search over a sorted, non-overlapping interval table.
bool inTable(uint32_t cp, const Range* t, int n) {
    int lo = 0, hi = n - 1;
    while (lo <= hi) {
        int mid = (lo + hi) / 2;
...
// Zero-width: combining marks, ZWSP/ZWNJ/ZWJ, variation selectors, etc.
const Range kZero[] = {
    ...
    {0x0A70,0x0A71},{0x0A75,0x0A75},{0x0ABC,0x0ABC},{0x0AC1,0x0AC5},
    ...
    {0x1B6B,0x1B73},{0x1B80,0x1B81},{0x1BA2,0x1BA5},{0x1BE6,0x1BE6},
...
constexpr int kZeroN = sizeof(kZero) / sizeof(kZero[0]);
constexpr int kWideN = sizeof(kWide) / sizeof(kWide[0]);
...
int charWidth(uint32_t cp) {
    ...
    if (inTable(cp, kZero, kZeroN)) return 0;
    if (inTable(cp, kWide, kWideN)) return 2;
    return 1;
}
